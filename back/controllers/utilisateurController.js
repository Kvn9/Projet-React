...
exports.updateUtilisateur = async (req, res) => {
    ...
    const { name, username, email } = req.body; // Ajout des variables manquantes
    const sql = 'CALL updateuser(?, ?, ?, ?)';
    await db.query(sql, [userId, name, username, email]); 
...
exports.insertUtilisateur = async (req, res) => {
    ...
    try {
        ...
        if (rows && rows.length > 0) {
            ...
            const sqlInsert = "INSERT INTO user (name, username, email, password) VALUES (?, ?, ?, ?)";
            await db.query(sqlInsert, [name, username, email, password ]);
...
exports.login = async(req, res)=> {
        ...
        const { email, password } = req.body 
        const result = await db.query('select email, password from user where email = ?', [email])
        if(result.length == 0){
